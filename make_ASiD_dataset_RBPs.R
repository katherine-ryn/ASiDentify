...
rbps <- cbind(rbps, ExcExpression = NA, InhExpression = NA, AstroExpression = NA, MicroExpression = NA, OligoExpression = NA)
ens_gene = c()
i = 1
for (i in 1:nrow(snHuman)) {
...
#Grep in the proteins by ENSG to rbps. 
ens_gene = c()
i = 1
  ...
  row_match = which(grepl(ens_id, rbps$Ensembl_ID))
   ...
   if (!identical(row_match, integer(0))) {
    rbps[row_match, c(24:30)] <- protein_expression[i,7:13]
    }
} 
...
#0 RNA in CBC:
i = 1
for (i in 1:nrow(rbps)){
  if (is.na(rbps[i,24])) {
    CBC_RNA = which(grepl(rbps[i,2], BrainSpan_RNA_expr$ensembl_gene_id))
    if (length(CBC_RNA) > 1) {
      print(CBC_RNA)
    }
...
sum(is.na(rbps$`log10(CBC)`)) #1544

#0 RNA in MD:
i = 1
for (i in 1:nrow(rbps)){
  if (is.na(rbps[i,25])) {
    MD_RNA = which(grepl(rbps[i,2], BrainSpan_RNA_expr$ensembl_gene_id))
    if (length(MD_RNA) > 1) {
      print(MD_RNA)
    }
...
for (i in 1:nrow(rbps)){
  if (is.na(rbps[i,26])) {
    STR_RNA = which(grepl(rbps[i,2], BrainSpan_RNA_expr$ensembl_gene_id))
    if (length(STR_RNA) > 1) {
      print(STR_RNA)
    }
...
sum(is.na(rbps$`log10(STR)`)) #1545

#0 RNA in AMY:
i = 1
for (i in 1:nrow(rbps)){
  if (is.na(rbps[i,27])) {
    AMY_RNA = which(grepl(rbps[i,2], BrainSpan_RNA_expr$ensembl_gene_id))
    if (length(AMY_RNA) > 1) {
      print(AMY_RNA)
    }
...
sum(is.na(rbps$`log10(AMY)`)) #1550

#0 RNA in HIP:
i = 1
for (i in 1:nrow(rbps)){
  if (is.na(rbps[i,28])) {
    HIP_RNA = which(grepl(rbps[i,2], BrainSpan_RNA_expr$ensembl_gene_id))
    if (length(HIP_RNA) > 1) {
      print(HIP_RNA)
    }
...
sum(is.na(rbps$`log10(HIP)`)) #1548

#0 RNA in V1C:
i = 1
for (i in 1:nrow(rbps)){
  if (is.na(rbps[i,29])) {
    V1C_RNA = which(grepl(rbps[i,2], BrainSpan_RNA_expr$ensembl_gene_id))
    if (length(V1C_RNA) > 1) {
      print(V1C_RNA)
    }
...
sum(is.na(rbps$`log10(V1C)`)) #1543

#0 RNA in DFC:
i = 1
for (i in 1:nrow(rbps)){
  if (is.na(rbps[i,30])) {
    DFC_RNA = which(grepl(rbps[i,2], BrainSpan_RNA_expr$ensembl_gene_id))
    if (length(DFC_RNA) > 1) {
      print(DFC_RNA)
    }
...
for (i in 1:nrow(LOEUF_no_dup)) {
  ...
  if (!identical(row_match, integer(0))) {
    rbps[row_match, c(31)] <- LOEUF_no_dup[i,30]
  }
} 
colnames(rbps)[31] <- c("LOEUF")
...
for (i in 1:nrow(synapse)) {
  ...
  if (!identical(row_match, integer(0))) {
    rbps[row_match, c(32)] <- 1
  } 
}
colnames(rbps)[32] <- c("Synapse")
...
for (i in 1:nrow(sfari)) {
  ...
  row_match = which(grepl(ens_id, rbps$Ensembl_ID))
    if (length(row_match) > 1) { print(ens_id) }
    if (!identical(row_match, integer(0))) {
      rbps[row_match, c(33)] <- 1
    } 
}
colnames(rbps)[33] <- c("Autism_susceptibility") 
...
for(i in names(ldf)){
  ...
  names(ldf[[i]]) <- c("Gene name",
                       ...
                       "Synapse localization",
                       "Autism susceptibility")
  ldf[[i]] <- impute(ldf[[i]], "mean")
}

i = 1
for(i in names(ldf)){
   write.table(ldf[[i]], str_glue("./model_input/RBP/{i}.tsv"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
}
