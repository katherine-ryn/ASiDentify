...
# Purpose: This script uses the nested_cv.R output from all PC genes and generates the AUROC/AUPRC curves and output supp. tables. 
#          
# ---
...
#Label 363 new candidate ASD risk genes, not found by 4 other papers or in bona fide ASD risk gene list:
...
for (r in 1:nrow(genes)) {
  if (genes[r,33] == 1 & genes[r,31] == 0 & !(genes[r,4] %in% union_top)) {
    genes[r,34] <- 1
  } else if (genes[r,33] != 1) {
    genes[r,34] <- NA
  } else (genes[r,34] <- 0)
}
...
asd_roc <- plotROC(autism_res, 31, 32)
asd_roc
asd_pr <- plotPR(autism_res, 31, 32)
asd_pr
...
svg(paste0("./figures/Figure_2.svg"), height = 3.5, width = 7.2)
print(Figure_2)
dev.off()
