...
#Randomly pick 10 sets of 82 ASD genes and 737 non-ASD genes.
set.seed(1567)
genes_set1_ASD <- genes %>% filter(`Autism susceptibility` == 1) %>% slice_sample(n = 82, replace = FALSE)
genes_set1_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
genes_set2_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
genes_set3_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
genes_set4_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
genes_set5_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
genes_set6_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
genes_set7_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
genes_set8_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
genes_set9_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
genes_set10_nonASD <- genes %>% filter(`Autism susceptibility` == 0) %>% slice_sample(n = 737, replace = FALSE)
...
set_list <- list(genes_set1, genes_set2, genes_set3, genes_set4, genes_set5, 
                 genes_set7, genes_set7, genes_set8, genes_set9, genes_set10)
i = 1
for (i in 1:10) {
  autism_res <- trainCV(set_list[[i]], K = 5, indep_var = "autism", standardized_col = c(6:29), feature_col = c(6:30))
